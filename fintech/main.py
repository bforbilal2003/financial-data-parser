...
def benchmark(df1: pd.DataFrame, df2: pd.DataFrame,
    ...
    for n in sizes:
        ...
        for r in range(repeats):
            ...
            rows.append({"TxnCount": len(txns), "Target": t, "BruteForce_s": bf_time, "DP_s": dp_time})
...
def save_benchmark_plots(bench_df: pd.DataFrame, out_dir: str) -> List[str]:
    ...
    plt.figure()
    g = bench_df.groupby("TxnCount").agg({"BruteForce_s": "mean", "DP_s": "mean"}).reset_index()
    plt.plot(g["TxnCount"], g["BruteForce_s"], marker="o", linestyle="--", label="Brute Force")
    plt.plot(g["TxnCount"], g["DP_s"], marker="s", linestyle="-", label="Dynamic Programming")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "bench_compare.png"), bbox_inches="tight")
    return paths
